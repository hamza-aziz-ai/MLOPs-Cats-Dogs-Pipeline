...
Student: Hamza Aziz, ID 2024AC05133, semester **S2-25** (confirmed by student;
...
- `models/resnet50_baseline.pt` exists on disk (~128 MB) but is **untracked**
  and reflects an early/partial run (best checkpoint saved as of epoch 7,
...
5. Provision a self-hosted runner for the deploy job — the previous one (WSL2
...
8. Replace the STALE EVIDENCE banners and every number in
   ...
   real regenerated values. Don't fabricate numbers — every one used this
...
- Got a full live CI/CD cycle green end-to-end against the *old* CatDogCNN:
  ...
  real GHCR publish + real remote deploy + real `/health`/`/predict` against
...
- Repo cleanup pass: removed a superseded MLflow run, empty default
...
- Extensive notebook-content pass: the ResNet-50 notebook was originally a
  ...
  rule that emerged: **describe what the current PyTorch code does, on its
...
- **Verify, don't assume.** Every claim in the docs was backed by an actual
...
- **No stale claims left standing.** Every time a change invalidated a
  ...
  behavior), it got corrected in the same pass, not left to rot — including
...
- **Commit locally, push only when asked.** Standing instruction, still in
  effect. Local commits are cheap and reversible; pushes are not (they
