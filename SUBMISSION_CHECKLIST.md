...
- [x] Split is 1,600 trains / 200 validations / 200 tests.
- [x] Images are 224 × 224 RGB; the corrupt count is zero.
...
- [x] Capture MLflow run/metrics/artefacts screenshot. (`docs/screenshots/E5_mlflow_run.png`, `E6_mlflow_artifacts.png`)
...
- [x] Create/select a GitHub repository: **https://github.com/hamza-aziz-ai/MLOPs-Cats-Dogs-Pipeline**.
- [x] Push final commit. Tag pending confirmed semester (see Identity sections).
...
- [x] Confirm the package exists in GHCR: **https://github.com/hamza-aziz-ai/MLOPs-Cats-Dogs-Pipeline/pkgs/container/mlops-cats-dogs-pipeline**.
- [x] Record immutable digest: **`sha256:8d6f10f8f231fe565944b5b4bd9bddd0a9a946438c879ccc8572d80578e2e0bf`**.
- [x] Verify displayed digest matches the image intended for M4 deployment (same tag/digest pulled by the deployment job).
- [x] Capture 8-passed/83%-coverage screenshot. (`docs/screenshots/E8_pytest_coverage.png`)
- [x] Capture a successful GitHub Actions test /build screenshot. (`docs/screenshots/E10_github_actions_run.png`)
...
- [x] The main-branch CD path is configured for a Linux x64 self-hosted runner.
- [x] Deployment logic pulls/deploys the image and runs a post-deployment smoke check.
- [x] The final local Compose stack was verified with both API and Prometheus containers healthy.
...
- [x] Confirm the deployed image digest: **`sha256:8d6f10f8f231fe565944b5b4bd9bddd0a9a946438c879ccc8572d80578e2e0bf`**.
...
- [x] Capture a request / response log screenshot without private data. (`docs/screenshots/E13a_request_logs.png`, `E13b_feedback_predictions.png`)
...
- [x] Verify ZIP contains the required source, configuration, and model artefact (`src/`, `scripts/`, `tests/`, `params.yaml`, `dvc.yaml`, `Dockerfile`, `models/cat_dog_cnn.pt`, `mlruns/`).
...
- [ ] Confirm the final video duration is below 5:00.
...
- [ ] Review the rendered report for broken links, diagrams, and page layout.
...
- [x] **E8 M3:** eight tests passed and 83% coverage. (`docs/screenshots/E8_pytest_coverage.png`)
...
- [x] **E11 M4:** both Compose containers are healthy. (`docs/screenshots/E11_compose_healthy.png`)
...
- [x] Image builds — the same image already lives on GHCR (see M3).
...
- [x] Replace every required to be bracketed placeholder (video URL/filename is the one remaining item, since the video itself doesn't exist yet).
...
- [x] Confirm model checksum and MLflow run ID match artefacts/screenshots.
- [x] Confirm the final Git commit / tag is the submitted one (`896c4cd`, tag `s2-25-aimlczg523-assignment2`).
- [x] Confirm all evaluator links open (GitHub run, GHCR package, deploy job all verified reachable).
- [ ] Watch the final video once at normal speed; audio/text are readable.
- [ ] Upload before the LMS deadline and retain the receipt.
